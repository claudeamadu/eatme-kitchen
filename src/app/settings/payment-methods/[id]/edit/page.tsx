...
export default function EditPaymentMethodPage() {
    ...
    useEffect(() => {
        if (user && typeof id === 'string') {
            const fetchWallet = async () => {
                ...
                if (docSnap.exists()) {
                    const data = docSnap.data() as Omit<wallet, 'id'>;
                    setFullName(data.name);
                    setNetwork(data.network);
                    setPhone(data.number);
                } else {
                    toast({ variant: 'destructive', title: 'Not Found', description: 'Could not find the selected payment method.' });
                    router.back();
                }
                setIsFetching(false);
            };
    ...
    const handleUpdateWallet = async () => {
        ...
        try {
            ...
            toast({ title: 'Wallet Updated', description: 'Your payment method has been updated.' });
            router.back();
        } catch (error: any) {
    ...
    return (
        <div className="food-pattern min-h-screen pb-24">
            <header className="p-4 flex items-center gap-4 sticky top-0 bg-transparent z-10">
                ...
                <h1 className="text-2xl font-headline font-bold">Edit Wallet</h1>
            </header>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-transparent">
                <Button size="lg" className="w-full max-w-md mx-auto rounded-full bg-red-600 hover:bg-red-700 text-white text-lg h-14" onClick={handleUpdateWallet} disabled={isLoading}>
